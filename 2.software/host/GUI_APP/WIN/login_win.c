...
//宏定义
#define LOGIN_ACCOUNT		"123456"	//登录手机号 
...
static void event_handler(lv_obj_t * obj,lv_event_t event)
{
  if(obj==account_ta||obj==password_ta)
  {
    if(event==LV_EVENT_RELEASED)//松手事件
		{
			lv_ta_set_cursor_type(account_ta,LV_CURSOR_LINE|LV_CURSOR_HIDDEN);//先隐藏光标
			lv_ta_set_cursor_type(password_ta,LV_CURSOR_LINE|LV_CURSOR_HIDDEN);//先隐藏光标
      kb_login_create(obj);//弹出键盘
		}
  }
	else if(obj==kb)
	{
	...
	else if(obj==login_btn)
	{
		if(event==LV_EVENT_RELEASED)
		{
			...
			const char * phone = lv_ta_get_text(account_ta);//获取输入的电话号码
...
void kb_login_create(lv_obj_t * bind_ta)
{
	...
	lv_kb_set_mode(kb,bind_ta==account_ta?LV_KB_MODE_NUM:LV_KB_MODE_TEXT);//如果绑定的是电话输入框,则弹出数字键盘,否则弹出文本键盘
...
void login_win_create(lv_obj_t * parent)
{
	...
	lv_obj_set_size(account_ta,parent_width*0.7f,30);//设置大小
	lv_obj_align(account_ta,logo_img,LV_ALIGN_OUT_BOTTOM_MID,0,20);//设置对齐方式
	lv_ta_set_text(account_ta,"");//清空默认的文本内容
	lv_ta_set_placeholder_text(account_ta,"Input account");//设置placeholder提示文本
	lv_ta_set_one_line(account_ta,true);//使能单行模式
	lv_obj_set_event_cb(account_ta,event_handler);//设置事件回调函数
	//创建一个密码输入框
	password_ta = lv_ta_create(login_win,account_ta);//从account_ta上进行拷贝
